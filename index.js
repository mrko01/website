...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      if (currentMode === "login") {
        ...
        if (!result.user.emailVerified) {
          await signOut(auth);
          showModal(
            "Your email is not verified. Please check your Gmail for a verification link and then try signing in again."
          );
        ...
        showModal(
          `A verification email has been sent to ${email}. Please check your Gmail, verify your account, then sign in.`
        );
        clearInputs();
  ...
  document.getElementById("forgot-password-link").addEventListener("click", async (e) => {
    ...
    const email = document.getElementById("email").value.trim();
    if (!email) {
      alert("Please enter your email address for password reset.");
      return;
    }
    try {
